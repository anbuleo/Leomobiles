...
function EditProduct() {
    ...
    let {EditProduct} = UseEditProduct()
    let params = useParams()
    let {getDataById,loadingHome,data} = UseGetProductByParams()
    let id = params.id
    ...
    let initVaL = {
      ...
      productType: {name:data?.productType.name||'',features:data?.productType.features||{}},
    ...
    const handleConvertUrl =async(val)=>{
      let urlimg = await convertToBase64(val)
      setImgUrl(urlimg)
      dispatch({type:'changeImage',nextVal:urlimg})
    }
  
    return (
      <>
        {loadingHome ?<><LoadingPage /></>:<><section className="border-stone-950 bg-stone-300 h-screen  ">
          <div className=" border-orange-600 flex justify-center pt-10">
            <div className="card bg-white w-3/4 ">
              <div className="">
                <p className="text-4xl font-bold text-center">Edit Product</p>
              </div>
              <div className="">
    ...
    className="textarea textarea-bordered textarea-accent textarea-lg  max-w-full" onChange={(e)=>dispatch({type:'changeDescription',nextVal: e.target.value})}></textarea>
                      ...
                      <button className="btn btn-outline btn-accent w-full "  type="submit">Submit Edited {state.productName}</button></div>
